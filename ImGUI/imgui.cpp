...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
	...
	float bars_width = 10.0f;//square_sz; // Arbitrary smallish width of Hue/Alpha picking bars (changes the rainbow bar width on the side of the color picker box)
	float box_size_change_factor = 120.0f;
	float sv_picker_size = ImMax(bars_width * 1, box_size_change_factor - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
